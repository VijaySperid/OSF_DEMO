...
export const SpecificationsWidgetCustom = () => import('../index');

describe('SpecificationsWidgetCustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {SpecificationsWidgetCustom};
  beforeAll(async () => {
    /**
  ...
  it('SpecificationsWidgetCustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'SpecificationsWidgetCustom',
            regions: []
          }
        }
      }
