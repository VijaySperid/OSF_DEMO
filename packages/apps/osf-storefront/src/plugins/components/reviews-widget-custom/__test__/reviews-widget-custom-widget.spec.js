...
export const ReviewsWidgetCustom = () => import('../index');

describe('ReviewsWidgetCustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ReviewsWidgetCustom};
  beforeAll(async () => {
    /**
  ...
  it('ReviewsWidgetCustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ReviewsWidgetCustom',
            regions: []
          }
        }
      }
