...
export const FooterwidgetServiceCustom = () => import('../index');

describe('FooterwidgetServiceCustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {FooterwidgetServiceCustom};
  beforeAll(async () => {
    /**
  ...
  it('FooterwidgetServiceCustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'FooterwidgetServiceCustom',
            regions: []
          }
        }
      }
