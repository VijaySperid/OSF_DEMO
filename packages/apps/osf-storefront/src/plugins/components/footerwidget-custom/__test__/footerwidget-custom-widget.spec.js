...
export const FooterwidgetCustom = () => import('../index');

describe('FooterwidgetCustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {FooterwidgetCustom};
  beforeAll(async () => {
    /**
  ...
  it('FooterwidgetCustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'FooterwidgetCustom',
            regions: []
          }
        }
      }
