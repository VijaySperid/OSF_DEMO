...
export const SimilarProductsWidgetCustom = () => import('../index');

describe('SimilarProductsWidgetCustom renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {SimilarProductsWidgetCustom};
  beforeAll(async () => {
    /**
  ...
  it('SimilarProductsWidgetCustom works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'SimilarProductsWidgetCustom',
            regions: []
          }
        }
      }
